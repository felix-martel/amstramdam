...
class GameMap:
    def __init__(self, name, map_id, udf,
                 harshness=None,
                 weights=None,
                 min_group=0,
                 max_group=None,
        ...
        self.map_id = map_id
        self.df = udf
        self.counts = Counter(udf.group)
        self.min_group = min_group
        if max_group is None:
            max_group = self.df.group.max()
        self.max_group = max_group
        # Samping weights
        self._weights = arg_or_default(weights, [1] * len(self.counts))
    ...
    def get_geometry(self, max_points=400):
        data = self.df
        if max_points > 0:
    ...
    def normalize_weights(self, weights):
        ...
        return Counter({group: weight/sum(weights) for group, weight in enumerate(weights)})

    def jsonify_dataset(self):
    ...
    def sample(self, k, verbose=True):
        ...
        points = self.df[mask].sample(k,
                                      replace=False,
                                      weights=probabilities).to_dict("records")
